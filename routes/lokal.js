...
router.get('/', funkcije.povuciPodatkeLokala, async(req, res, next)=>{
    let korisnik = false;
    if(typeof(req.cookies.cookie_sesija) !== 'undefined') {await funkcije.postaviTipKorisnika(req, res, ()=>{});korisnik = req.tip; }
    res.render('lokal', {lokal: req.lokal, korisnik})
} )

router.post('/proposal',upload.single("image"), funkcije.predlloziUmjetniku);
